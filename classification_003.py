...
#        cls = soft_voting_ensemble()
        ...
        res.append(name)
        print('Модель обучена')
        accuracy_evaluation(X_ready, y, cls)
...
def accuracy_evaluation(X, y, cls):
    scores = cross_val_score(cls, X, y, scoring='accuracy', cv=10)

    from contextlib import redirect_stdout
    with open('result_accuracy_14_08_2018.txt', 'a') as f:
        with redirect_stdout(f):
#    cls_scores = np.sqrt(scores)
            print('Оценки точности', scores)
